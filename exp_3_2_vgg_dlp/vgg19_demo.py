...
class VGG19(object):
    ...
    def load_model(self):
        ...
        for idx in range(self.net.size()):
            if 'conv' in self.net.getLayerName(idx):
                ...
                weight = np.moveaxis(weight, 3, 0).flatten().astype(np.float)
            ...
            if 'fc' in self.net.getLayerName(idx):
                ...
                weight = np.transpose(weight, [0, 1]).flatten().astype(np.float)
                bias = bias.reshape(-1).astype(np.float)
    ...
    def load_image(self, image_dir):
        ...
        # TODO：调整输入数据
        input_data = np.moveaxis(input_image, 2, 1).flatten().tolist()
