...
class TransNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.layer = nn.Sequential(
            ...
            ###################下采样层################
            # TODO：构建图像转换网络，第一层卷积
            nn.Conv2d(3, 32, kernel_size=9, stride=1, padding=4, bias=False),  # 假设输入通道数为3
            # TODO：实例归一化
...
if __name__ == '__main__':
    ...
    example_input = torch.rand((1, 3, 512, 512), dtype=torch.float)
    net_trace = torch.jit.trace(net, example_input)
    for i, image in enumerate(data_group):
        ...
        save_image(torch.cat((image_c, image_g_mlu), 2), f'./out/mlu_cnnl_mfus/image_{i}.jpg')
