...
class ConvolutionalLayer(object):
    ...
    def forward_speedup(self, input):
        ...
        self.col_image = im2col(self.input_pad, self.kernel_size,
                                self.stride)  # N,Cin,H,W -> N,(height_out)*(width_out),cin*k*k
        ...
        self.output = np.reshape(self.output, np.hstack(([N], [height_out], [width_out], [
            cout])))  # N,hight_out*width_out,Cout -> N,hight_out, width_out, Cout->N,Cout,hight_out,width_out
                ...
                for idxh in range(height_out):
                    for idxw in range(width_out):
                        ...
                        self.output[idxn, idxc, idxh, idxw] = np.max(
                            self.input[idxn, idxc, hs:hs + self.kernel_size, ws:ws + self.kernel_size])
