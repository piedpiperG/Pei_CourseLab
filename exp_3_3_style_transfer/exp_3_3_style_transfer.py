...
class VGG19(object):
    ...
    def load_image(self, image_dir, image_height, image_width):
        ...
        # 调整图像尺寸
        input_image = input_image.resize((image_width, image_height), Image.Resampling.LANCZOS)

        # 将图像转换为numpy数组，并调整数据类型
        ...
        # 调整输入数据的形状以符合 [N, channel, height, width]
        input_image = np.transpose(input_image, [0, 3, 1, 2])
        return input_image, input_image.shape
    ...
    def save_image(self, input_image, image_shape, image_dir):
        ...
        output_image = output_image.resize((image_shape[1], image_shape[0]), Image.Resampling.LANCZOS)
